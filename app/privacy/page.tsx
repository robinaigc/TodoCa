...
export default function PrivacyPage() {
  return (
    <LegalPage title="Privacy 隐私政策" updatedAt="2026 年 7 月 10 日">
      <section>
        <h2 className="font-semibold text-text-primary">1. TodoCa 保存哪些数据</h2>
        <p className="mt-2">
          你的用户画像（例如身份、登陆日期和家庭情况），以及任务的完成、跳过和进度数据，保存在当前浏览器的 localStorage 中。这些本地数据不会因为你使用任务功能而自动上传到 TodoCa 的服务器。
        </p>
        <p className="mt-2">
          localStorage 只属于当前浏览器和设备。更换设备、使用其他浏览器、无痕模式结束，或清除浏览器数据，都可能导致本地画像和进度永久丢失。TodoCa V1 不提供账户或云同步恢复。
        </p>
      </section>
      ...
      <section>
        ...
        <p className="mt-2">
          当你主动使用 /help 协助表单时，表单可能收集姓名或昵称、邮箱、微信号或其他你在问题描述中提供的联系方式，以及问题类别和描述。
        </p>
        <p className="mt-2">
      ...
      <section>
        ...
        <p className="mt-2">
          设备本地的用户画像和任务进度可在“我的”页面使用“清除所有数据”删除；浏览器设置也可以清除 TodoCa 的站点数据。本机暂存的协助线索需要通过浏览器站点数据设置清除。
        </p>
      </section>
    </LegalPage>
  );
